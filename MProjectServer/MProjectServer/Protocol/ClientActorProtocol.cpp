...
void NC2S_MoveActorInWorldProtocolHandler::OnReceivePacket(SessionKey _session_key, std::unique_ptr<FPacket> _packet) {
	...
	if (true == user.expired()) {
		LogManager::GetMutableInstance().GenericLog(ELogLevel::Warning, "NC2S_MoveActorInWorldProtocolHandler", "OnReceivePacket", std::format("User not find. [{}]", _session_key));
		return;
	}
	...
	if (true == world.expired()) {
		LogManager::GetMutableInstance().GenericLog(ELogLevel::Warning, "NC2S_MoveActorInWorldHandler", "OnReceivePacket", "World is not find.");
		return;
	}

	auto game_player = user.lock()->GetGamePlayer();
	if (true == game_player.expired()) {
		LogManager::GetMutableInstance().GenericLog(ELogLevel::Warning, "NC2S_MoveActorInWorldProtocolHandler", "OnReceivePacket", std::format("GPC is expired. [{}]", user.lock()->GetUserKey()));
		return;
	}
	for (auto iter = data->actors()->begin(); iter != data->actors()->end(); ++iter) {
		if (auto actor = game_player.lock()->GetActor(iter->actor_key()); false == actor.expired()) {
			actor.lock()->SetTransform(FTransform(iter->transform()));
		}
	}
}
